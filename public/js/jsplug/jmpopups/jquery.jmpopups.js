...
(function($) {
		...
		if (name) {
			for (var i = 0; i < openedPopups.length; i++) {
				if (openedPopups[i].name == name) {
					var thisPopup = openedPopups[0];
					
					openedPopups.splice(i,1)
					
					thisPopup.beforeClose();

					$("#popupLayer_" + name).hide(0,function(){
	...
	function setPopupLayersPosition(popupElement, animate) {
		if (popupElement) {
			...
			if(top_tmp==0)
				top_tmp=document.body.scrollTop;
			
			if (popupElement.height() < $(window).height()) {
				var topPosition = top_tmp + ($(window).height() - popupElement.height()) / 2;
			} else {
				var topPosition = top_tmp + 5;
			}

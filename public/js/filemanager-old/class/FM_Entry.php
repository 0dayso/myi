...
/* PROTECTED METHODS *************************************************************************** */
	...
	protected function _getIcon() {
		...
		if($this->type == 'cdup') return 'folder';
		$ext = end(explode('.', $this->name));
		...
		$icon = $this->FileManager->getIconDir() . '/big/file_extension_' . $ext . '.png';
		if(is_file($icon)) return 'file_extension_' . $ext;
		$icon = $this->FileManager->getIconDir() . '/big/file_extension_' . $this->type . '.png';
		return is_file($icon) ? 'file_extension_' . $this->type : 'document_yellow';
	}

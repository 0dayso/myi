		...
		if($fp = @fopen($this->_path, 'r')) {
	        ...
	        if($header['signature'] != 'ID3') {
	   		...
	   		for($i = 0; $i < 22; $i++) {
				...
				if($tag && isset($this->_id3Tags[$tag])) {
					if($tag == 'COMM') {
						if(preg_match('/^.*?[\x00-\x1A]+(.+)$/', $value)) {
							$value = $m[1];
						}
					}

					if($tag == 'APIC') {
						if(preg_match('/image\/([a-z]+)[\x00-\x1A]*(.+)/s', $value, $m)) {
					...
					if($value != '') $value = preg_replace('/[\x00-\x1A]+/', '', $value);

					$id3[$this->_id3Tags[$tag]] = $value;
				}
			}
			@fclose($fp);
	  		return $id3;

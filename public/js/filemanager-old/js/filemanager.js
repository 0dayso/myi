...
var fmLib =  {
	...
	viewResponse: function(json, curCont, listLoad) {
		...
		if(typeof json == 'object') {
			...
			var folderId = (json.reloadExplorer == 'all') ? 0 : json.reloadExplorer;
			if(!json.login) this.getExplorer(json.cont, '', '', folderId);
							...
							ajaxObj.makeRequest(url + '&fmMode=getMessages', function() {
								...
								if(doResize) {
									fmTools.addListener(window, 'resize', function() {
										...
										fmLib.resizeTimeout = setTimeout('fmLib.initFileManager("' + url + '", "' + mode + '")', 250);
									});
								}
	...
	setFileManagerSize: function(curCont) {
		var fmCont = $(curCont);
		var listCont = $(curCont + 'List');
	...
	openDialog: function(url, dialogId, text, fileId, x, y) {
		var f, e, i, obj, sid, tmp, curCont, iframeCont, dialog, perc, dir, width, height, name, perms;
		...
		if(dialogId == 'fmMediaPlayer') {
			obj = $('fmMediaCont');
			fmParser.parseMediaPlayer({curCont:curCont,id:fileId,url:url}, obj);
		}
			...
			default:
				if(id >= 0) {
					...
					if(fmEntries[curCont][id].deleted) {
						...
						else if(fmEntries[curCont][id].name.match(fmParser.imageFiles)) {

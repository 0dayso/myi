...
class Icwebadmin_RepoSrepController extends Zend_Controller_Action
{
	private $_filter;
	private $_mycommon;
    ...
    public function indexAction(){
    	if($_GET['sdata']){
    		...
    		$this->view->soprod = $this->_repoService->samplesRepo($sql);
    	}
    	//销售
    ...
    public function exportAction(){
    	...
    	if($_GET['sdata']){
    		$newname = "IC_samplesorder_".$_GET['sdata']."_".$_GET['edata'].".xls";
    		...
    		$soprod = $this->_repoService->samplesRepo($sql);
    		if($soprod){
    			//生成新excel
    			...
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(3,1,"项目名称");
    			$objExcel->getActiveSheet()->getColumnDimension('E')->setWidth(18);
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(4,1,"订单号");
    			$objExcel->getActiveSheet()->getColumnDimension('F')->setWidth(18);
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(5,1,"申请时间");
    			$objExcel->getActiveSheet()->getColumnDimension('I')->setWidth(12);
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(6,1,"品牌");
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(7,1,"型号");
    			
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(8,1,"数量");
    			$objExcel->getActiveSheet()->getColumnDimension('J')->setWidth(35);
    			$objExcel->getSheet(0)->setCellValueByColumnAndRow(9,1,"备注");
    			...
    			foreach($soprod as $v){
    				...
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(3,$row1,$v['projectname']);
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(4,$row1,$v['salesnumber']);
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(5,$row1,date('Y/m/d',$v['created']));
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(6,$row1,$v['brandname']);
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(7,$row1,$v['part_no']);
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(8,$row1,$v['approvenum']);
    				$objExcel->getSheet(0)->setCellValueByColumnAndRow(9,$row1,$v['remark']);
    				$row1++;
    			}
    			$objWriter = new PHPExcel_Writer_Excel5($objExcel);
    			$objWriter->save('php://output');
    		}
    	}else $this->_redirect('/icwebadmin/RepoSrep');
    }
}